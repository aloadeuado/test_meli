// This file was generated from JSON Schema using quicktype, do not modify it directly.
// To parse the JSON, add this file to your project and do:
//
//   let shipping = try Shipping(json)

import Foundation

// MARK: - Shipping
struct Shipping: Codable {
    var storePickUp, freeShipping: Bool?
    var logisticType: LogisticType?
    var mode: Mode?
    var tags: [ShippingTag]?
    var promise: JSONNull?

    enum CodingKeys: String, CodingKey {
        case storePickUp = "store_pick_up"
        case freeShipping = "free_shipping"
        case logisticType = "logistic_type"
        case mode, tags, promise
    }
}

// MARK: Shipping convenience initializers and mutators

extension Shipping {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(Shipping.self, from: data)
    }

    init(_ json: String, using encoding: String.Encoding = .utf8) throws {
        guard let data = json.data(using: encoding) else {
            throw NSError(domain: "JSONDecoding", code: 0, userInfo: nil)
        }
        try self.init(data: data)
    }

    init(fromURL url: URL) throws {
        try self.init(data: try Data(contentsOf: url))
    }

    func with(
        storePickUp: Bool?? = nil,
        freeShipping: Bool?? = nil,
        logisticType: LogisticType?? = nil,
        mode: Mode?? = nil,
        tags: [ShippingTag]?? = nil,
        promise: JSONNull?? = nil
    ) -> Shipping {
        return Shipping(
            storePickUp: storePickUp ?? self.storePickUp,
            freeShipping: freeShipping ?? self.freeShipping,
            logisticType: logisticType ?? self.logisticType,
            mode: mode ?? self.mode,
            tags: tags ?? self.tags,
            promise: promise ?? self.promise
        )
    }

    func jsonData() throws -> Data {
        return try newJSONEncoder().encode(self)
    }

    func jsonString(encoding: String.Encoding = .utf8) throws -> String? {
        return String(data: try self.jsonData(), encoding: encoding)
    }
}
